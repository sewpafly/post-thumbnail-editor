...
"');}return __;")}function x(b,c){b._wrap=A(b,true,a.isArray(c)?c:[u.test(c)?c:a(c).html()]).join("")}function B(b){return b?b.replace(/\\'/g,"'").replace(/\\\\/g,"\\"):null}function r(b){function c(E){function M(L){L+=f;v=o[L]=o[L]||D(v,l[v.parent.key+f]||v.parent)}var C,y=E,v,I;if(I=E.getAttribute(p)){for(;y.parentNode&&(y=y.parentNode).nodeType===1&&!(C=y.getAttribute(p)););if(C!==I){y=y.parentNode?y.nodeType===11?0:y.getAttribute(p)||0:0;if(!(v=l[I])){v=s[I];v=D(v,l[y]||s[y]);v.key=++n;l[n]=v}e&&
M(I)}E.removeAttribute(p)}else if(e&&(v=a.data(E,"tmplItem"))){M(v.key);l[v.key]=v;y=(y=a.data(E.parentNode,"tmplItem"))?y.key:0}if(v){for(C=v;C&&C.key!=y;){C.nodes.push(E);C=C.parent}delete v._ctnt;delete v._wrap;a.data(E,"tmplItem",v)}}var f="_"+e,d,k,o={},t,w,G;t=0;for(w=b.length;t<w;t++)if((d=b[t]).nodeType===1){k=d.getElementsByTagName("*");for(G=k.length-1;G>=0;G--)c(k[G]);c(d)}}function K(b,c,f,d){if(!b)return q.pop();q.push({_:b,tmpl:c,item:this,data:f,options:d})}function g(b,c,f){return a.tmpl(a.template(b),
c,f,this)}function i(b,c){var f=b.options||{};f.wrapped=c;return a.tmpl(a.template(b.tmpl),b.data,f,b.item)}function j(b,c){var f=this._wrap;return a.map(a(a.isArray(f)?f.join(""):f).filter(b||"*"),function(d){if(c)d=d.innerText||d.textContent;else{var k;if(!(k=d.outerHTML)){k=document.createElement("div");k.appendChild(d.cloneNode(true));k=k.innerHTML}d=k}return d})}function h(){var b=this.nodes;a.tmpl(null,null,null,this).insertBefore(b[0]);a(b).remove()}var m=a.fn.domManip,p="_tmplitem",u=/^[^<]*(<[\w\W]+>)[^>]*$|\{\{\! /,
...
"string")c=H(c);else if(c instanceof a)c=c[0]||{};if(c.nodeType)c=a.data(c,"tmpl")||a.data(c,"tmpl",H(c.innerHTML));return typeof b==="string"?a.template[b]=c:c}return b?typeof b!=="string"?a.template(null,b):a.template[b]||a.template(null,u.test(b)?b:a(b)):null},encode:function(b){return(""+b).split("<").join("&lt;").split(">").join("&gt;").split('"').join("&#34;").split("'").join("&#39;")}});a.extend(a.tmpl,{tag:{tmpl:{_default:{$2:"null"},open:"if($notnull_1){__=__.concat($item.nest($1,$2));}"},
...
return i.setTimeout(function(){if(!(j("iframe",h).height()>m)){j("iframe",h).css({height:m});log("THICKBOX: "+h.width()+" x "+h.height());return true}},1E3)}}}})(x);B=function(g){return{}.toString.call(g).match(/\s([a-z|A-Z]+)/)[1].toLowerCase()};D=function(){function g(i){this.message=i;this.date=new Date}g.prototype.toString=function(){var i,j,h,m,p,u,l;u=function(s,z){for(;(""+s).length<z;)s="0"+s;return s};h=this.date;l=u(h.getUTCFullYear(),4);j=u(h.getUTCMonth()+1,2);i=u(h.getUTCDate(),2);m=
u(h.getUTCHours(),2);p=u(h.getUTCMinutes(),2);u=u(h.getUTCSeconds(),2);switch(B(this.message)){case "string":h=this.message;break;default:h=a.toJSON(this.message)}return""+l+j+i+" "+m+":"+p+":"+u+" - ["+B(this.message)+"] "+h};return g}();(function(g){g.messages=[];g.log=function(i){if(!r.options.pte_debug)return true;try{g.messages.push(new D(i));console.log(i)}catch(j){}return true};g.formatLog=function(){var i,j,h,m,p;i="";p=g.messages;h=0;for(m=p.length;h<m;h++){j=p[h];i+=""+j+"\n"}return i};
g.parseServerLog=function(i){var j,h,m;log("===== SERVER LOG =====");h=0;for(m=i.length;h<m;h++){j=i[h];log(j)}return true};g.sendToPastebin=function(i){return a.ajax({url:"http://dpastey.appspot.com/",data:{title:"PostThumbnailEditor Log",content:i,lexer:"text",format:"json",expire_options:"2592000"},dataType:"json",global:false,type:"POST",error:function(j,h,m){a("#pte-log").fadeOut("900");alert("Sorry, there was a problem trying to send to pastebin");log(j);log(h);return log(m)},success:function(j){a("#pte-log").fadeOut("900");
...
g("#pte-log-messages textarea").val(x.formatLog());return g("#pte-log").fadeIn("900")})});(function(g){return g.admin=function(){var i,j,h,m,p,u;u="&TB_iframe=true&height="+r.options.pte_tb_height+"&width="+r.options.pte_tb_width;h=null;p=function(l){l=l||h||a("#attachment-id").val();return""+ajaxurl+"?action=pte_ajax&pte-action=launch&id="+l+u};i=function(l){return a('<a class="thickbox" href="'+p(l)+'">'+objectL10n.PTE+"</a>")};j=function(l){log("Start PTE...");if(r.parent.frames.length>0){log("Modifying thickbox...");
...
log(j);return x.parseServerLog(j.log)}})})})})};H=function(g,i){if(g===0)return i;for(;i>0;)if(g>i)g-=i;else i-=g;if(g<0||i<0)return null;return g};F=function(g,i){var j,h,m,p,u;h=thumbnail_info[i];j=h.crop;p=h.width;m=h.height;j=+j;p=+p;m=+m;h=H(p,m);if(j!=null&&j>0){j=(u=r.options.pte_ar[i])!=null?u:null;if(j==null&&(p!=null)>0&&(m!=null)>0)j=h!=null?""+p/h+":"+m/h:""+p+":"+m;if(g!=null&&j!=null&&j!==g)throw"Too many Aspect Ratios. Disabling";return j}};(function(g){var i,j,h,m,p,u,l,s,z,J;g.editor=
function(){var n,e;J();h();z();j();m();p();i();n=a("#pte-loading");e=function(){n.hide();return true};a("#pte-preview").load(e);n.ajaxStart(function(){return a(this).fadeIn(200)}).ajaxStop(function(){return a(this).fadeOut(200)});r.setTimeout(e,2E3);return true};J=function(){var n;n=new A(function(){var e;log("===== REFLOW =====");g.fixThickbox(r.parent);e=a("#pte-sizes").offset();e=a(r).height()-e.top-2;return a("#pte-sizes").height(e)},100);a(r).resize(n.doFunc).load(n.doFunc);return true};h=function(){var n;
n=function(e){e.delegate("tr","click",function(q){q.target.type!=="checkbox"&&a("input:checkbox",this).click();return true});return e.delegate("input:checkbox","click",function(){this.checked||a(this).is("input:checked")?a(this).parents("tr").first().removeClass("selected"):a(this).parents("tr").first().addClass("selected");return true})};n(a("#stage2"));return n(a("#stage1"))};s=null;l={keys:true,minWidth:3,minHeight:3,handles:true,zIndex:1200,instance:true,onSelectEnd:function(n,e){return e.width&&
e.width>0&&e.height&&e.height>0&&a(".pte-size").filter(":checked").size()>0?a("#pte-submit").removeAttr("disabled"):a("#pte-submit").attr("disabled",true)}};z=function(){return s=a("#pte-image img").imgAreaSelect(l)};u=function(n){log("===== SETTING ASPECTRATIO: "+n+" =====");s.setOptions({aspectRatio:n});return s.update()};j=function(){var n;n=new A(function(){var e;e=null;a("input.pte-size").filter(":checked").each(function(q,b){try{e=F(e,a(b).val())}catch(c){e=null;e!==s.getOptions().aspectRatio&&
alert(c);return false}return true});u(e);l.onSelectEnd(null,s.getSelection());return true},50);return a("input.pte-size").click(n.doFunc)};m=function(){var n;a("#pte-submit").click(function(){var e,q;q=s.getSelection();e=a("#pte-sizer").val();e={id:a("#pte-post-id").val(),action:"pte_ajax","pte-action":"resize-images","pte-sizes[]":a(".pte-size").filter(":checked").map(function(){return a(this).val()}).get(),x:Math.floor(q.x1/e),y:Math.floor(q.y1/e),w:Math.floor(q.width/e),h:Math.floor(q.height/e)};
log("===== RESIZE-IMAGES =====");log(e);s.setOptions({hide:true,x1:0,y1:0,x2:0,y2:0});a("#pte-submit").attr("disabled",true);a.getJSON(ajaxurl,e,n);return true});return n=function(e){log("===== RESIZE-IMAGES SUCCESS =====");log(e);g.parseServerLog(e.log);if(e.error!=null&&e.thumbnails==null)alert(e.error);else{a("#stage1").animate({left:-a(r).width()},500,"swing",function(){a(this).hide();a("#stage2").html(a("#stage2template").tmpl(e)).show(0,function(){a(this).animate({left:0},500);return true});
return true});return false}}};p=function(){var n;a("#pte-confirm").live("click",function(){var e,q;q={};a("input.pte-confirm").filter(":checked").each(function(b,c){var f;f=a(c).val();return q[f]=a("#pte-"+f+"-file").val()});e={id:a("#pte-post-id").val(),action:"pte_ajax","pte-action":"confirm-images","pte-nonce":a("#pte-nonce").val(),"pte-confirm":q};log("===== CONFIRM-IMAGES =====");log(e);return a.getJSON(ajaxurl,e,n)});return n=function(e){log("===== CONFIRM-IMAGES SUCCESS =====");log(e);g.parseServerLog(e.log);
a("#stage2").animate({left:-a(r).width()},500,"swing",function(){a(this).hide();a("#stage3").html(a("#stage3template").tmpl(e)).show(0,function(){a(this).animate({left:0},500);return true});return true});return false}};return i=function(){var n,e;e=function(q){var b,c;q!=null&&q.preventDefault();q=(b=(c=q.data)!=null?c.selector:void 0)!=null?b:".pte-size";return a(q).filter(":checked").click()};n=function(q){var b,c;q!=null&&q.preventDefault();q=(b=q!=null?(c=q.data)!=null?c.selector:void 0:void 0)!=
null?b:".pte-size";return a(q).not(":checked").click()};a("#pte-selectors .all").click(n);a("#pte-selectors .none").click(e).click();a("#stage2").delegate("#pte-stage2-selectors .all","click",{selector:".pte-confirm"},n);a("#stage2").delegate("#pte-stage2-selectors .none","click",{selector:".pte-confirm"},e);return true}})(x)}).call(this);
