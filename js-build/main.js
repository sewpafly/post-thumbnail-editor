...
(function() {
  ...
  define('cs!controllers/PteCtrl',['angular', 'cs!apps/pteApp', 'cs!settings', 'cs!jquery'], function(angular, app, settings, $) {
    app.controller("PteCtrl", [
      '$scope', '$resource', '$log', function($scope, $resource, $log) {
        /*
        ...
        */
        ...
        $scope.save = function(thumbnail_array) {
          var confirm_results, data, key, thumbnail, _i, _len;
          event.stopPropagation();
          data = {
          ...
          confirm_results = $scope.thumbnailResource.get(data, function() {
            ...
            for (_j = 0, _len1 = thumbnail_array.length; _j < _len1; _j++) {
              thumbnail = thumbnail_array[_j];
              if (thumbnail.selected) {
        ...
        */
        ...
        $scope.trash = function(thumbnail) {
          ...
          return thumbnail.showProposed = false;
        };
        ...
        deleteTemp = function() {
          var deleteResults;
          if (nonces['pte-delete-nonce'] == null) {
            return;
          }
...
(function() {
  ...
  define('cs!jcrop-api',['cs!jquery', 'cs!settings', 'jcrop', 'domReady!'], function($, settings) {
    ...
    crop_options = {
      ...
      trueSize: [settings.getWindowVar('post_width'), settings.getWindowVar('post_height')]
    };
    $("#pte-preview").Jcrop(crop_options, function() {
      jcrop = this;
      if (jcrop != null) {
        jcrop.release();
      }
        ...
        $scope.updateSelected = function() {
          ...
          try {
            angular.forEach($scope.thumbnails, function(thumbnail) {
              var crop, height, tmp_ar, width;
              crop = thumbnail.crop, width = thumbnail.width, height = thumbnail.height;
              crop = +crop;
              width = +width;
              height = +height;
              if (thumbnail.selected && thumbnail.crop > 0) {
                tmp_ar = width / height;
